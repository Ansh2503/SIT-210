...
The checkButton() function watches for when the button is pressed and starts the timers for the lights so they only turn on once per press. The porchLight() function turns on the porch LED for 30 seconds and then turns it off automatically. The hallLight() function turns on the hallway LED for 60 seconds and then turns it off automatically. By separating these tasks into different functions, each part of the program does only one job, which makes it easier to read, test, and change. Both lights can stay on at the same time for their own durations, so when Linda presses the button, the porch and hallway lights turn on immediately to help her enter the house safely. 
